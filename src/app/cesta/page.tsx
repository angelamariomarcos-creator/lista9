...
export default function CestaPage() {
  ...
  async function handleRemove(id: string) {
    const supabase = createClient();
    const { error } = await supabase.from("cesta").delete().eq("id", id);
  ...
  async function handleToggle(id: string, comprado: boolean) {
    const supabase = createClient();
    const { error } = await supabase.from("cesta").update({ comprado }).eq("id", id);
    if (error) console.error("Error al marcar comprado:", error);
  }
  ...
  async function handleVaciar() {
    ...
    if (error) console.error("Error al vaciar cesta:", error);
    setVaciando(false);
  }
